...
export const Post = ({
    ...
    React.useEffect(() => {
        ...
        const handleCommentatorId = async () => {
            try {
    ...
    const onClickRemove = () => {
        if (window.confirm('Вы дейтсвительно хотите удалить статью?')) {
                ...
                <div className={styles.indention}>
                    <h2 className={clsx(styles.title, { [styles.titleFull]: isFullPost })}>
                        {isFullPost ? (
                            ...
                            <Link to={`/posts/${id}`} state={{ id, userId: commentatorId }}>
                                {title}
                            </Link>
                        )}
                    </h2>
